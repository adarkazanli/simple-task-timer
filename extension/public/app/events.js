$(function() {
    ...
    $(window).resize(function() {
        ...
        if (task_open) {
            $('#task-menu').css({ left: ((($(window).width() - $('#task-menu').outerWidth(true)) / $(window).width()) * 100).toString() + '%' });
        }
        if (tools_open) {
            $('#tools-menu').css({ left: ((($(window).width() - $('#tools-menu').outerWidth(true)) / $(window).width()) * 100).toString() + '%' });
        }
        check_width();
    });
    ...
    $('#new-btn').click(function() {
        ...
        var hours = parseInt($('#new-goal-hours').val());
        var mins = parseInt($('#new-goal-mins').val());
        var indef = $('#new-goal-indef').is(':checked');
        if ($('#new-txt').val().trim() != '' && (hours > 0 || mins > 0 || indef)) {
            // Add the task to the array
            cancel_edit();
            var start = task_count > 0 ? new Date(tasks[task_count - 1].end) : new Date();
            ...
            if ($('#new-start').is(':checked')) {
                toggle_task(task_count - 1);
            }
            SaveTasks();
    ...
    $('.close-menus').click(function() {
        ...
        task_open = false;
        displaying_task = -1;
        if (!alarm_open && !task_open && !tools_open && !dialog_open) {
            $('#modal').fadeOut(600);
        }
    ...
    $('#close-alarm').click(function() {
        ...
        if (!task_open && !tools_open && !dialog_open) {
            $('#modal').fadeOut(600);
        }
    });
    ...
    $('#tools-button').click(function() {
        ...
        $('#modal').fadeIn(600, function() {
            $('#tools-pulsate').stop(true, true).fadeOut(400);
        });
    ...
    $('#csv-export').click(function() {
        ...
        a.download = 'tasks_' + new Date().toString().replace(/\s{1,}/g, '_') + '.csv';
        a.click();
    });
    ...
    $('#preview-sound').click(function() {
        ...
        if ($('#sound-type').val() == 1 || $('#custom-sound').val() != '') {
            preview_sound = true;
            $('#preview').attr('src', $('#sound-type').val() == 1 ? '/ui/fx/deneb.ogg' : $('#custom-sound').val());
