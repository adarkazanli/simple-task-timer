    ...
    try {
        if (editing_task === -1) {
            ...
            if (task != editing_task) {
                error(locale('errFinishEditing', tasks[editing_task].text));
            }
        }
    } catch (e) {
        js_error(e);
...
function save_current(task) {
    try {
        ...
        var hours = parseInt($('#current-edit-' + task + ' .hrs').val());
        var mins = parseInt($('#current-edit-' + task + ' .mins').val());
        var secs = parseInt($('#current-edit-' + task + ' .secs').val());
        // Set the goal
...
function save_goal(task) {
    try {
        ...
        var hours = parseInt($('#goal-edit-' + task + ' .hrs').val());
        var mins = parseInt($('#goal-edit-' + task + ' .mins').val());
        var secs = parseInt($('#goal-edit-' + task + ' .secs').val());
        var indef = $('#goal-edit-' + task + ' .indef').is(':checked');
