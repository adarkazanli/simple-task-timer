    ...
    try {
        ...
        for (var i = 0; i < task_count; i++) {
            if (task_running[i]) {
                ...
                if (tasks[i].current_secs >= 60) {
                    tasks[i].current_secs -= 60;
                    tasks[i].current_mins++;
                }
                if (tasks[i].current_mins >= 60) {
                    tasks[i].current_mins -= 60;
                    tasks[i].current_hours++;
                }
                // Historical time
                if (Setting('track-history')) {
                    // Make sure the object exists all the way down
                    if (typeof tasks[i].history == 'undefined') {
                        tasks[i].history = {};
                    }
                    if (typeof tasks[i].history[year] == 'undefined') {
                        tasks[i].history[year] = {};
                    }
                    if (typeof tasks[i].history[year][month] == 'undefined') {
                        tasks[i].history[year][month] = {};
                    }
                    if (typeof tasks[i].history[year][month][day] == 'undefined') {
                        tasks[i].history[year][month][day] = {};
                    }
                    if (typeof tasks[i].history[year][month][day][hour] == 'undefined') {
                        tasks[i].history[year][month][day][hour] = { secs: 0, mins: 0, hours: 0 };
                    }
                    // Increment the historical time
                    tasks[i].history[year][month][day][hour].secs++;
                    if (tasks[i].history[year][month][day][hour].secs >= 60) {
                        tasks[i].history[year][month][day][hour].secs -= 60;
                        tasks[i].history[year][month][day][hour].mins++;
                    }
                    if (tasks[i].history[year][month][day][hour].mins >= 60) {
                        tasks[i].history[year][month][day][hour].mins -= 60;
                        tasks[i].history[year][month][day][hour].hours++;
                    }
                }
                // Task meets its goal
                if (!tasks[i].indefinite && tasks[i].current_hours >= tasks[i].goal_hours && tasks[i].current_mins >= tasks[i].goal_mins) {
                    ...
                    if (Setting('no-overtime')) {
                        ...
                        $('tr#task-' + i + ' img.toggle').attr('src', '/ui/images/tasks/control_play.png').addClass('disabled');
                    }
                    ...
                    if (!tasks[i].notified) {
                        ...
                        if (Setting('notify') && webkitNotifications.checkPermission() == 0) {
                            var notification = webkitNotifications.createNotification('/ui/images/app/icon-64.png', locale('noteTaskFinished', tasks[i].text), locale('noteTaskFinishedLong', tasks[i].text));
                ...
                if (displaying_task == i) {
                    task_info(i, false, progress);
                }
            }
        }
        // Update totals row
    ...
    try {
        dialog(locale('confResetTask', tasks[task].text), function(status, data) {
            ...
            if (status) {
                ...
                if (displaying_task == task) {
                    $('#task-toggle').removeAttr('disabled');
                }
            }
        ...
        dialog(locale('confDeleteTask', tasks[task].text), function(status, data) {
            ...
            if (status) {
                ...
                setTimeout(function() {
                    ...
                    if (task_count >= 2) {
                        $('table#task-list tfoot').fadeIn();
                    } else {
                        $('table#task-list tfoot').fadeOut();
                    }
                }, 20);
                SaveTasks();
                $('#new-txt').focus();
...
function toggle_task(task) {
    try {
        if (task_running[task]) {
            ...
            $('#task-' + task + ' img.toggle').attr('title', locale('btnStart')).attr('src', '/ui/images/tasks/control_play_blue.png');
            if (displaying_task == task) {
                $('#task-toggle').text(locale('btnStart'));
            }
            ...
            $('#task-' + task + ' img.toggle').attr('title', locale('btnStop')).attr('src', '/ui/images/tasks/control_pause_blue.png');
            if (displaying_task == task) {
                $('#task-toggle').text(locale('btnStop'));
            }
...
function list_task(task, anim) {
    try {
        ...
        // Text
        $('#task-' + task + ' td.text .label').text(tasks[task].text);
        var start = new Date(tasks[task].start);
        var end = new Date(tasks[task].end);
        $('#task-' + task + ' td.text .time .start').text(format_time(start.getHours(), start.getMinutes(), null, null, 'h:mm a'));
        $('#task-' + task + ' td.text .time .end').text(format_time(end.getHours(), end.getMinutes(), null, null, 'h:mm a'));
        ...
        $('#task-' + task + ' img.toggle').attr('title', task_running[task] ? locale('btnStop') : locale('btnStart')).attr('src', '/ui/images/tasks/control_' + (task_running[task] ? 'pause' : 'play') + '_blue.png');
        // Progress bar
        ...
        $('#task-' + task + ' .info').attr('name', task).click(function() {
            if (!$(this).hasClass('disabled')) {
                task_info(parseInt($(this).attr('name')), true, true, false);
            }
        });
        ...
        if (!tasks[task].indefinite && tasks[task].current_hours >= tasks[task].goal_hours && tasks[task].current_mins >= tasks[task].goal_mins) {
            if (Setting('no-overtime')) {
                ...
                $('#task-' + task + ' img.toggle').attr('src', '/ui/images/tasks/control_play.png').addClass('disabled');
            }
            $('#task-' + task).addClass('done');
        }
        // Update task menu if it's shown for this task
        if (displaying_task == task) {
            task_info(task, false, false, progress);
        }
        // Animation
        if (anim == 0) {
            // Show instantly
...
function task_info(task, user_triggered, anim, progress) {
    try {
        ...
        if (task_running[task]) {
            $('button#task-toggle').text(locale('btnStop'));
        } else {
            $('button#task-toggle').text(locale('btnStart'));
        }
        if ($('tr#task-' + task + ' button.toggle').attr('disabled')) {
        ...
        if (typeof tasks[displaying_task].history != 'undefined' && typeof tasks[displaying_task].history[y] != 'undefined' && typeof tasks[displaying_task].history[y][m - 1] != 'undefined' && typeof tasks[displaying_task].history[y][m - 1][d] != 'undefined') {
            ...
            for (h = 0; h <= 23; h++) {
                if (typeof tasks[displaying_task].history[y][m - 1][d][h] != 'undefined') {
                    $('<tr />').append('<td>' + (Setting('12-hour') ? (h == 0 ? '12' : (h > 12 ? h - 12 : h)) : h) + ':00' + (Setting('12-hour') ? (h >= 12 ? ' PM' : ' AM') : '') + '</td>').append('<td>' + format_time(tasks[displaying_task].history[y][m - 1][d][h].hours, tasks[displaying_task].history[y][m - 1][d][h].mins, tasks[displaying_task].history[y][m - 1][d][h].secs) + '</td>').appendTo('#history tbody');
                }
            }
        } else {
...
function TaskSetting(id, task, value, only_not_exists) {
    ...
    if (!value && ((exists && !only_not_exists) || (!exists && only_not_exists))) {
        // Set the setting
