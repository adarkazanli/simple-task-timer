...
function LoadSettings(reset_timer, from_save) {
    ...
    if (!Setting('translate-hidden', false, true)) {
    ...
    if (Setting('sound-type', 1, true) == 2) {
        ...
        $('#sound').attr('src', '/ui/fx/deneb.ogg');
    }
...
function SaveSettings() {
    ...
    if (Setting('notify')) {
        webkitNotifications.requestPermission(function() {
            webkitNotifications.createNotification('/ui/images/app/icon-64.png', locale('noteNotifsWork'), locale('noteNotifsWorkBody')).show();
        });
    }
    // Reset the small window alerted state
    if (old_use_icons != Setting('use-icons') && !Setting('use-icons')) {
