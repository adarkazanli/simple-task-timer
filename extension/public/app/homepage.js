...
var autosaves = 0;
var dragging = false;
var preview_sound = false;
var now = new Date(); // General variables
var alarm_open = false;
var task_open = false;
var tools_open = false;
var dialog_open = false; // Menu state variables
var js_error_shown = false;
var no_local_files_alerted = false; // Alert state variables
var current_plot = false;
var total_plot = false; // Plot variables
var save_timer;
var timer;
var timer_step = 0; // Timer variables
var DEBUG = false; // Debug mode

// Set error event (most important event)
window.onerror = function(msg, url, line) {
    js_error(msg, url, line);
};
...
$(function() {
    try {
        ...
        if (lang != 'en' && lang != 'en-CA' && lang != 'en-GB' && lang != 'en-US') {
            $('#translations-accuracy').show();
        }
        ...
        if (localStorage['tasks']) {
            ...
            for (var i = 0; i < task_count; i++) {
                ...
                if (typeof tasks[i].indefinite == 'undefined') {
                    tasks[i].indefinite = false;
                }
                // Add the description property to a task if it doesn't exist
                if (typeof tasks[i].description == 'undefined') {
                    tasks[i].description = '';
                }
                // Add the settings property to a task if it doesn't exist or is null
                if (typeof tasks[i].settings == 'undefined' || tasks[i].settings == null) {
                    tasks[i].settings = {};
                }
                LoadTaskSettings(i);
                // Make sure goal times aren't null
                if (tasks[i].goal_hours == null) {
                    tasks[i].goal_hours = 0;
                }
                if (tasks[i].goal_mins == null) {
                    tasks[i].goal_mins = 0;
                }
                list_task(i, 0);
        ...
        if (Setting('autostart-default')) {
            $('#new-start').attr('checked', 'checked');
        }
        if (Setting('save-fields')) {
            ...
            if (Setting('field-start', false, true)) {
                $('#new-start').attr('checked', 'checked');
            }
        ...
        $('table#task-list').tableDnD({
            ...
            onDrop: function(table, row) {
                ...
                var tmp2 = task_running[old_id];
                if (typeof tasks[old_id] != 'undefined') {
